...
- **`GROUP_CONCAT`/`SAMPLE`** — compute over the fetched rows client-side. *(`SUM`/`AVG`/`MIN`/`MAX` were originally planned the same way; this analysis was superseded — SHQL's `aggregate:` block now supports them directly, pushed down to the storage layer for single-pattern queries, so they are exact at any scale. See the conversion plan, §5 Phase 4.)*
...
**The catch:** this trades server-side efficiency for engineering safety, and it has a real ceiling. SHQL's internal node/edge search caps a single pattern's candidate fetch at 2,000 documents by default (configurable via `HGAI_SHQL_MAX_NODE_CANDIDATES` / `HGAI_SHQL_MAX_EDGE_CANDIDATES`; overflow is reported as `meta.truncated` in the result). Server-side `COUNT`/`group_by` over a single pattern is now exact and uncapped (`meta.aggregate_pushdown`), as are `SUM`/`AVG`/`MIN`/`MAX` (the SHQL `aggregate:` block now supports them); what remains capped is any client-side computation over fetched rows, such as `GROUP_CONCAT`, `SAMPLE`, output-only `BIND` and multi-pattern aggregates regardless of the `limit:` the query itself requests. A transpiler that fetches "everything" to aggregate client-side is implicitly relying on the true match count staying under that ceiling — past it, the aggregation is *incomplete* (flagged by `meta.truncated`), not merely slow. This is an acceptable tradeoff for the exploratory, small-to-medium-graph queries this conversion feature is realistically aimed at, but it must be documented as a known limitation, not quietly assumed away. That server-side extension has since been done for `COUNT`/`SUM`/`AVG`/`MIN`/`MAX` over single patterns.
...
| `SUM`/`AVG`/`MIN`/`MAX` | High — the most common aggregate functions after `COUNT` | **Built into SHQL** — `aggregate: {sum, avg, min, max}`, storage-pushed-down (exact) for single-pattern queries, in-memory (candidate-capped, flagged `meta.truncated`) otherwise | N/A | **Done.** The client-side approach was dropped once the storage layer gained native aggregation. |
