...
@pytest.mark.asyncio
async def test_federated_shql_merges_results():
    mesh_doc = {
        "id": "m1",
        "servers": [
            # Non-empty graphs so _graphs_for_server's cached-list branch
            # short-circuits, and the query fan-out is the only HTTP call
            # this test needs to mock (fetch_remote_graphs's own .get()
            # call is a separate, unmocked code path otherwise).
            {"server_id": "s1", "server_name": "S1", "url": "http://host-a:8357",
             "graphs": ["g1"], "status": "active", "tags": [], "attributes": {},
    ...
    # _query_server calls get_http_client().post(...) directly (the shared,
    # lazily-created AsyncClient — see hgai_module_mesh/engine.py's "Shared
    # HTTP client" section), not a freshly-instantiated per-request client,
    # so that's the accessor to mock rather than httpx.AsyncClient itself.
    mock_client = AsyncMock()
...
@pytest.mark.asyncio
async def test_federated_shql_mesh_not_found():
    mock_mesh_store = MagicMock()
